...
export const unifiedResponse = function (response?: IResponse) {
  ...
  if (!isStr(msg)) {
    httpResponse.msg = httpResponse.error
      ? "系统错误"
...
export default class HttpServer {
  ...
  // 响应拦截
  responseIntercept: IHttpServerOptons["responseIntercept"] = (response) =>
    response;
  ...
  upload(url: string, filePath, data: object | null, header = {}, options?) {
    return new Promise((resolve: (res: HttpResponse) => void, reject) => {
      ...
      this.uploadFile({
        ...
        filePath: isObj(filePath)
          ? filePath.tempFilePath || filePath.path
          : filePath,
        name,
        formData: data || {},
        header: getHeader(header, this.getHeader ? this.getHeader() : {}),
